                                      ...
                                      const Size size) const
{
    ...
    if (own_pid < pid)
        snprintf(buf, size, "/Nicoos.%u.%u", own_pid, pid);
    else
        snprintf(buf, size, "/Nicoos.%u.%u", pid, own_pid);
}

void HostShareManager::setReady(const bool ready) const
{
    char readyFile[32];
    snprintf(readyFile, sizeof(readyFile), "/Nicoos.%u.ready", getpid());
...
bool HostShareManager::isReady(const ProcessID pid) const
{
    ...
    snprintf(readyFile, sizeof(readyFile), "/Nicoos.%u.ready", pid);
